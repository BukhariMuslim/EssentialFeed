...
class FeedUIIntegrationTests: XCTestCase {
    ...
    func test_feedView_hasTitle() throws {
    ...
    func test_imageSelection_notifiesHandler() throws {
        let image0 = makeImage()
        let image1 = makeImage()
        var selectedImages = [FeedImage]()
        let (sut, loader) = try makeSUT(selection: { selectedImages.append($0) })
    ...
    func test_loadFeedActions_runsAutomaticallyOnlyOnFirstAppearance() throws {
        let (sut, loader) = try makeSUT()
        XCTAssertEqual(loader.loadFeedCallCount, 0, "Expected no loading requests before view appears")
    ...
    func test_loadFeedCompletion_dispatchesFromBackgroundToMainThread() throws {
        let (sut, loader) = try  makeSUT()
        sut.simulateAppearance()
    ...
    func test_loadFeedCompletion_rendersErrorMessageOnErrorUntilNextReload() throws {
        let (sut, loader) = try makeSUT()
        
        sut.simulateAppearance()
        
        XCTAssertEqual(sut.errorMessage, nil, "Expected no error message initially")
    ...
    func test_loadMoreActions_requestMoreFromLoader() throws {
        let (sut, loader) = try makeSUT()
        sut.simulateAppearance()
        ...
        sut.simulateLoadMoreFeedAction()
        XCTAssertEqual(loader.loadMoreCallCount, 1, "Expected a loading request once view appears")
    ...
    func test_loadingMoreIndicator_isVisibleWhileLoadingMore() throws {
        let (sut, loader) = try makeSUT()
    ...
    func test_loadMoreCompletion_dispatchesFromBackgroundToMainThread() throws {
        let (sut, loader) = try makeSUT()
        sut.simulateAppearance()
        loader.completeFeedLoading(at: 0)
        sut.simulateLoadMoreFeedAction()
    ...
    func test_loadMoreCompletion_rendersErrorMessageOnError() throws {
        let (sut, loader) = try makeSUT()
        sut.simulateAppearance()
    ...
    func test_tapOnLoadMoreErrorView_loadsMore() throws {
        let (sut, loader) = try makeSUT()
        sut.simulateAppearance()
        loader.completeFeedLoading()
    ...
    func test_feedImageViewRetryButton_isVisibleOnImageURLLoadError() throws {
        ...
        loader.completeImageLoadingWithError(at: 1)
        XCTAssertEqual(view0?.isShowingRetryAction, false, "Expected retry action image state change for first view once second image loading completes successfully")
    ...
    func test_loadImageDataCompletion_dispatchesFromBackgroundToMainThread() throws {
        let (sut, loader) = try makeSUT()
    ...
    func test_feedImageView_doesNotLoadImageAgainUntilPreviousRequestCompletes() throws {
    ...
    private func makeSUT(
        ...
        line: UInt = #line
    ) throws -> (sut: ListViewController, loader: LoaderSpy) {
